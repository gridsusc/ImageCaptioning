...
class attention_net(nn.Module):
	...
	def forward(self,enc_image,decoder_out):
		...
		added_for_final_linear_layer = self.act(image_for_attention + decoder_out_for_attention.unsqueeze(1))

		passed_through_final_layer = self.final_linear_layer(added_for_final_linear_layer)
...
class decoder_with_attention_net(nn.Module):
	def __init__(self,attention_size,one_hot_size,embedding_size,dropout_probab):
		...
		self.lstm_cell = nn.LSTMCell(embedding_size + encoder_dim, 512)
	...
	def forward(self,enc_image,enc_captions,caps_len):
		...
		enc_image = enc_image.view(bs,-1,2048) # Making the enc_image (batches,14*14,2048)
		n_pixels = enc_image.size(dim=1)
		sorted_indices_list = sorted(list(range(len(caps_len))),key=lambda a:-caps_len[a])
		caps_len = [caps_len[i] for i in sorted_indices_list]
		sorted_enc_image = enc_image[sorted_indices_list]
		sorted_enc_captions = enc_captions[sorted_indices_list]
		embedding_values = self.embedding_layer(sorted_enc_captions)
		...
		predictions = torch.zeros(bs, biggest_caption-1, self.one_hot_size).to(device)
        all_alphas = torch.zeros(bs, biggest_caption-1, n_pixels).to(device)

		for i in range(biggest_caption-1): ## To avoid the last <eos> token being decoded
			n_samples_to_be_processed = 0
			for el in caps_len:
				if (el-1)>i: ## el-1 because we had to get rid of <eos> token
					n_samples_to_be_processed+=1

			alphas_for_each_pixel = self.attention_net(sorted_enc_image[:n_samples_to_be_processed],hidden_state[:n_samples_to_be_processed])
			gating_scalar = self.layer_to_get_gating_scalar(h[:n_samples_to_be_processed])
			gating_scalar = self.sigmoid_act(gating_scalar)
			encoding_with_attention = gating_scalar*((sorted_enc_image[:n_samples_to_be_processed]*alphas_for_each_pixel.unsqueeze(2)).sum(dim=1))
			hidden_state, cell_state = self.lstm_cell(storch.cat([embedding_values[:n_samples_to_be_processed, i, :], encoding_with_attention], dim=1), \
				(hidden_state[:n_samples_to_be_processed], cell_state[:n_samples_to_be_processed]))
